#!/usr/bin/env python3
"""
Calculate JSD (Jensen-Shannon Divergence) for Pre-SGO Predictions
==================================================================

This script:
1. Loads configuration from wandb (config.yaml)
2. Downloads pre_sgo_context and pre_sgo_without_context artifacts from W&B
3. For each review, calculates JSD between:
...
Usage:
    python Metrics and analysis/scripts/calculate_jsd_pre_sgo.py
"""
...
def process_review(review: Dict[str, Any], is_baseline: bool = False) -> Optional[Dict[str, Any]]:
    """
    Process a single review and calculate JSD.
    ...
    Returns:
        Dictionary with JSD and metadata, or None if processing failed
    """
    try:
        ...
        if is_baseline:
            ...
            if not actual_themes:
                # Fallback to themes list (convert to uniform distribution)
                themes_list = actual.get('themes', [])
                if themes_list:
                    actual_themes = {theme: 1.0 / len(themes_list) for theme in themes_list}
                else:
...
def process_json_file(file_path: Path, is_baseline: bool = False) -> List[Dict[str, Any]]:
    """
    Process a single JSON file and extract all reviews.
    
    Args:
        file_path: Path to JSON file
        is_baseline: If True, processes baseline file format (flat dict with review keys)
        
    Returns:
        List of processed review results
    """
    try:
        ...
        if is_baseline:
            # Baseline format: flat dict with keys like "user_id_review_index"
            for review_key, review in data.items():
                ...
                if 'user_id' not in review:
                    # Try to extract from key format: "user_id_review_index"
                    parts = review_key.rsplit('_review_', 1)
                    if len(parts) == 2:
                        review['user_id'] = parts[0]
                
                result = process_review(review, is_baseline=True)
                if result:
                    results.append(result)
        else:
            # Standard format: nested structure with user_predictions
            ...
            for user_id, reviews in user_predictions.items():
                ...
                for review in reviews:
                    ...
                    result = process_review(review, is_baseline=False)
                    if result:
                        results.append(result)
...
def process_artifact_directory(artifact_path: Path, max_reviews: Optional[int] = None, add_jsd_to_files: bool = True, filter_cluster: Optional[str] = None, is_baseline: bool = False) -> Tuple[List[Dict[str, Any]], List[Path]]:
    """
    ...
    Args:
        ...
        add_jsd_to_files: If True, add JSD metric to each review in the original files
        filter_cluster: Optional cluster ID to filter (e.g., "cluster_1"). If None, processes all clusters.
        
    Returns:
        Tuple of (list of per-review JSD results, list of updated file paths)
    """
    all_results = []
    updated_files = []
    ...
    for json_file in tqdm(json_files, desc="Processing files"):
        ...
        # Path format: .../cluster_X/micro_Y_summary_*.json
        ...
        if is_baseline:
            ...
            for review_key, review in data.items():
                ...
                # Calculate per-review JSD
                review_result = process_review(review, is_baseline=True)
                if review_result:
                    jsd_value = review_result['jsd']
                    
                    # Add JSD to the review in the original data structure
                    if add_jsd_to_files:
                        if 'metrics' not in review:
                            review['metrics'] = {}
                        review['metrics']['jsd'] = float(jsd_value)
                    
                    all_results.append(review_result)
            
            # Save updated file if JSD was added
            if add_jsd_to_files:
                try:
                    with open(json_file, 'w', encoding='utf-8') as f:
                        json.dump(data, f, indent=2, ensure_ascii=False)
                    updated_files.append(json_file)
                    logger.debug(f"Updated baseline file with JSD: {json_file}")
                except Exception as e:
        ...
        # Process each review for both approaches
        for user_id, reviews in user_predictions.items():
            if not isinstance(reviews, list):
                continue
            ...
            for review in reviews:
                ...
                # Get theme distributions (same logic as process_review)
                if is_baseline:
                    # Baseline: Try topic_probabilities first, then themes (list), then predicted_themes
                    actual_themes = actual.get('topic_probabilities', {})
                    if not actual_themes:
                ...
                # Calculate per-review JSD
                review_result = process_review(review)
                if review_result:
                    jsd_value = review_result['jsd']
        ...
        # Save updated file if JSD was added
        if add_jsd_to_files and file_updated:
            try:
                with open(json_file, 'w', encoding='utf-8') as f:
                    json.dump(data, f, indent=2, ensure_ascii=False)
                updated_files.append(json_file)
                logger.debug(f"Updated file with JSD: {json_file}")
            except Exception as e:
                logger.warning(f"Failed to update file {json_file}: {e}")
    ...
    if add_jsd_to_files:
        logger.info(f"Updated {len(updated_files)} files with JSD metrics")
...
def create_visualizations(
    ...
    """
    Create visualization graphs for per-review JSD analysis.
    
    Args:
        results: List of per-review JSD results
    ...
    # ============================================================================
    # APPROACH 1: Per-Review JSD (then aggregate statistics)
    # ============================================================================
    if results:
        ...
        # Calculate per-tribe JSD statistics
        tribe_jsd = defaultdict(list)
        cluster_tribes = defaultdict(dict)  # cluster_id -> {tribe_id: jsd_list}
        cluster_all_jsds = defaultdict(list)  # cluster_id -> [all_jsds] for direct averaging
        ...
        for r in results:
            ...
            jsd_val = r.get('jsd')
            if tribe_id and cluster_id and jsd_val is not None:
                tribe_jsd[tribe_id].append(jsd_val)
                if tribe_name:
                    tribe_names_map[tribe_id] = tribe_name
                ...
                cluster_tribes[cluster_id][tribe_id].append(jsd_val)
                # Also collect all JSDs for cluster-level averaging (from individual reviews)
                cluster_all_jsds[cluster_id].append(jsd_val)
        
        # Calculate mean JSD per tribe (average of individual review JSDs)
        tribe_mean_jsd = {tribe: np.mean(jsds) for tribe, jsds in tribe_jsd.items() if len(jsds) >= 3}
        tribe_review_counts = {tribe: len(jsds) for tribe, jsds in tribe_jsd.items()}
        
        # Calculate mean JSD per tribe per cluster (average of individual review JSDs)
        cluster_tribe_mean_jsd = {}
        cluster_tribe_counts = {}
        for cluster_id, tribes_dict in cluster_tribes.items():
            cluster_tribe_mean_jsd[cluster_id] = {}
            cluster_tribe_counts[cluster_id] = {}
            for tribe_id, jsds in tribes_dict.items():
                if len(jsds) >= 3:
                    # Average of individual review JSDs in this tribe
                    cluster_tribe_mean_jsd[cluster_id][tribe_id] = np.mean(jsds)
                    cluster_tribe_counts[cluster_id][tribe_id] = len(jsds)
        ...
        # ========================================================================
        # Graph 1: JSD Distribution Histogram (with percentage)
        # ========================================================================
        # Create bins for JSD values
        jsd_min, jsd_max = jsd_values.min(), jsd_values.max()
        bins = np.linspace(jsd_min, jsd_max, 30)
        bin_centers = (bins[:-1] + bins[1:]) / 2
        
        # Calculate histogram
        hist, _ = np.histogram(jsd_values, bins=bins)
        hist_percent = (hist / len(jsd_values)) * 100  # Convert to percentage
        ...
        bars = ax.bar(bin_centers, hist_percent, width=(bins[1]-bins[0])*0.8, 
                     ...
                     label='Real vs Synthetic JSD')
        
        # Add mean and median lines
        mean_percent = np.interp(mean_jsd, bin_centers, hist_percent)
        ax.axvline(mean_jsd, color='#FF6B6B', linestyle='--', linewidth=2.5, 
                  label=f'JSD(Mean): {mean_jsd:.4f}', zorder=5)
        ...
        ax.set_title(f'JSD Distribution: Real vs Synthetic\n{prefix.replace("_", " ").title()} (n={len(jsd_values):,} reviews)', 
        ...
        plt.tight_layout()
        filename = f"jsd_distribution.png"
        plt.savefig(artifact_output_dir / filename, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        logger.info(f"Saved: {filename}")
        
        # ========================================================================
        # Graph 2: Per-Tribe JSD Comparison (Grouped by Cluster)
        # ========================================================================
        if cluster_tribe_mean_jsd and len(cluster_tribe_mean_jsd) > 0:
            # Get all clusters and keep them in numerical order (1, 2, 3, 4, 5...)
            clusters = sorted(cluster_tribe_mean_jsd.keys())
            num_clusters = len(clusters)
            
            # Calculate number of tribes per cluster to determine sizing
            tribes_per_cluster = {cid: len(cluster_tribe_mean_jsd[cid]) for cid in clusters}
            ...
            for cluster_id in clusters:
                # Calculate cluster average directly from all individual review JSDs in that cluster
                if cluster_id in cluster_all_jsds and len(cluster_all_jsds[cluster_id]) > 0:
                    cluster_avg_jsd[cluster_id] = np.mean(cluster_all_jsds[cluster_id])
                else:
                    # Fallback: if no direct data, use tribe means (shouldn't happen)
                    cluster_tribes_data = cluster_tribe_mean_jsd[cluster_id]
                    if cluster_tribes_data:
                        cluster_avg_jsd[cluster_id] = np.mean(list(cluster_tribes_data.values()))
                    else:
                        cluster_avg_jsd[cluster_id] = 0.0
            ...
            for idx, cluster_id in enumerate(clusters):
                ...
                # Sort tribes by JSD value (ascending - lowest first)
                ...
                # Add JSD value and review count labels - adjust font size and spacing
                if tribe_jsd_vals:
                    max_jsd = max(tribe_jsd_vals)
                    y_padding = max_jsd * 0.05  # More padding to prevent overlap
                    ...
                    for i, (bar, jsd_val, count) in enumerate(zip(bars, tribe_jsd_vals, tribe_counts)):
                        ...
                        # Add JSD value on top of bar with more spacing
                ...
                # Y-axis styling (JSD)
                ax.set_ylabel('Mean JSD', fontsize=14, fontweight='bold', color='#333333', labelpad=20)
                if tribe_jsd_vals:
                    ax.set_ylim(0, max(tribe_jsd_vals) * 1.25)  # More top margin for labels
                    ax.set_yticks(np.linspace(0, max(tribe_jsd_vals), 6))
                    ax.set_yticklabels([f'{y:.2f}' for y in np.linspace(0, max(tribe_jsd_vals), 6)], 
                                      fontsize=12)
                
                # Title styling with average JSD
                cluster_num = cluster_id.replace('cluster_', '') if cluster_id else '?'
                avg_jsd = cluster_avg_jsd.get(cluster_id, 0.0)
                ...
                if num_tribes > 20:
                    ...
                    title_fontsize = 15
                    title_pad = 20
                # Get cluster average from cluster_avg_jsd
                cluster_avg = cluster_avg_jsd.get(cluster_id, avg_jsd)
                ax.set_title(f'Cluster {cluster_num} ({len(tribe_labels)} tribes)\nCluster Avg: {cluster_avg:.4f}', 
            ...
            tribe_stats = {
                tribe: {
                    ...
                    'review_count': len(jsds),
                    'min_jsd': float(np.min(jsds)),
                    'max_jsd': float(np.max(jsds))
                }
                for tribe, jsds in tribe_jsd.items() if len(jsds) >= 3
            }
            tribe_stats_file = artifact_output_dir / "per_tribe_statistics.json"
            with open(tribe_stats_file, 'w', encoding='utf-8') as f:
                json.dump(tribe_stats, f, indent=2)
            logger.info(f"Saved: per_tribe_statistics.json")
        ...
        if actual_sum > 0 and predicted_sum > 0:
            ...
            ax.set_title(f'Theme Distributions: Real vs Synthetic\n{prefix.replace("_", " ").title()} (n={total_reviews_for_dist:,} reviews, Mean JSD: {mean_jsd:.4f})', 
                        fontsize=14, fontweight='bold', pad=15)
        ...
        stats_approach1 = {
            ...
            'num_tribes': len(tribe_mean_jsd) if tribe_mean_jsd else 0
        }
    ...
    # Print summary
    logger.info("\n" + "=" * 80)
    logger.info(f"JSD Results Summary - {prefix.replace('_', ' ').title()}")
    logger.info("=" * 80)
    if results:
        logger.info(f"Per-Review JSD: Mean = {mean_jsd:.6f}, Median = {median_jsd:.6f}")
...
def create_cluster_wise_comparison(
    ...
    """
    Create cluster-wise JSD comparison across all methods.
    ...
    # Aggregate JSD by cluster for each artifact
    cluster_data = defaultdict(lambda: defaultdict(list))  # cluster_id -> artifact_key -> [jsds]
    cluster_labels = []
    ...
    for artifact_key, results in all_results.items():
        for result in results:
            ...
            jsd = result.get('jsd')
            if cluster_id and jsd is not None:
                cluster_data[cluster_id][artifact_key].append(jsd)
    ...
    for artifact_key in artifact_keys:
        ...
        for cluster_id in all_clusters:
            jsds = cluster_data[cluster_id][artifact_key]
            artifact_all_jsds.extend(jsds)
        artifact_avg = np.mean(artifact_all_jsds) if artifact_all_jsds else 0.0
    ...
    for idx, artifact_key in enumerate(artifact_keys):
        ...
        for cluster_id in all_clusters:
            jsds = cluster_data[cluster_id][artifact_key]
            cluster_means.append(np.mean(jsds) if jsds else 0.0)
    ...
    ax.set_title(f'JSD Comparison by Cluster\n{avg_text}', 
...
def create_tribe_wise_comparison(
    ...
    """
    Create tribe-wise JSD comparison within each cluster.
    ...
    # Aggregate JSD by cluster and tribe for each artifact
    cluster_tribe_data = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    # cluster_id -> tribe_id -> artifact_key -> [jsds]
    tribe_names_map = {}  # tribe_id -> tribe_name
    ...
    for artifact_key, results in all_results.items():
        for result in results:
            ...
            jsd = result.get('jsd')
            if cluster_id and tribe_id and jsd is not None:
                cluster_tribe_data[cluster_id][tribe_id][artifact_key].append(jsd)
    ...
    for cluster_id in sorted(cluster_tribe_data.keys()):
        ...
        avg_text_parts = []
        for artifact_key in artifact_keys:
            artifact_cluster_jsds = []
            for tribe_id in tribes:
                jsds = tribe_data[tribe_id][artifact_key]
                artifact_cluster_jsds.extend(jsds)
            artifact_avg = np.mean(artifact_cluster_jsds) if artifact_cluster_jsds else 0.0
            display_name = get_artifact_display_name(artifact_key)
            avg_text_parts.append(f"{display_name} Avg: {artifact_avg:.4f}")
        avg_text = "\n".join(avg_text_parts)
        ...
        for idx, artifact_key in enumerate(artifact_keys):
            ...
            for tribe_id in tribes:
                jsds = tribe_data[tribe_id][artifact_key]
                tribe_means.append(np.mean(jsds) if jsds else 0.0)
        ...
        ax.set_ylabel('Mean JSD', fontsize=12, fontweight='bold')
        ax.set_title(f'JSD Comparison by Tribe: {cluster_id.replace("cluster_", "Cluster ")}\n{avg_text}', 
                    fontsize=14, fontweight='bold', pad=20)
        ax.set_xticks(x)
    ...
    for cluster_id in sorted(cluster_tribe_data.keys()):
        ...
        for tribe_id in sorted(cluster_tribe_data[cluster_id].keys()):
            ...
            for artifact_key in artifact_keys:
                jsds = cluster_tribe_data[cluster_id][tribe_id][artifact_key]
                if jsds:
...
def create_pre_post_sgo_tribe_comparison(
    ...
    # Aggregate JSD by cluster and tribe
    cluster_tribe_pre = defaultdict(lambda: defaultdict(list))  # cluster_id -> tribe_id -> [jsds]
    cluster_tribe_post = defaultdict(lambda: defaultdict(list))  # cluster_id -> tribe_id -> [jsds]
    tribe_names_map = {}  # tribe_id -> tribe_name
    ...
    for result in pre_sgo_results:
        ...
        jsd = result.get('jsd')
        if cluster_id and tribe_id and jsd is not None:
            cluster_tribe_pre[cluster_id][tribe_id].append(jsd)
            if tribe_name and tribe_id not in tribe_names_map:
                tribe_names_map[tribe_id] = tribe_name
    ...
    for result in post_sgo_results:
        ...
        jsd = result.get('jsd')
        if cluster_id and tribe_id and jsd is not None:
            cluster_tribe_post[cluster_id][tribe_id].append(jsd)
            if tribe_name and tribe_id not in tribe_names_map:
                tribe_names_map[tribe_id] = tribe_name
    ...
    for cluster_id in all_clusters:
        ...
        for tribe_id in all_tribes:
            pre_cluster_jsds.extend(pre_tribes.get(tribe_id, []))
            post_cluster_jsds.extend(post_tribes.get(tribe_id, []))
        pre_avg = np.mean(pre_cluster_jsds) if pre_cluster_jsds else 0.0
        post_avg = np.mean(post_cluster_jsds) if post_cluster_jsds else 0.0
        ...
        for tribe_id in all_tribes:
            pre_jsds = pre_tribes.get(tribe_id, [])
            post_jsds = post_tribes.get(tribe_id, [])
            
            # Only include if both have data
            if pre_jsds and post_jsds:
                ...
                pre_means.append(np.mean(pre_jsds))
                post_means.append(np.mean(post_jsds))
        ...
        ax.set_xlabel('Tribe', fontsize=12, fontweight='bold')
        ax.set_ylabel('Mean JSD', fontsize=12, fontweight='bold')
        ax.set_title(f'Pre-SGO vs Post-SGO JSD by Tribe: {cluster_id.replace("cluster_", "Cluster ")}\n{avg_text}', 
                    fontsize=14, fontweight='bold', pad=20)
        ax.set_xticks(x)
...
def process_single_artifact(
    ...
    Args:
        ...
        add_jsd_to_files: Whether to add JSD to original files
    ...
    results, updated_files = process_artifact_directory(
        artifact_path, max_reviews, add_jsd_to_files=add_jsd_to_files, is_baseline=is_baseline
    )
    ...
    if add_jsd_to_files:
        logger.info(f"Added JSD metrics to {len(updated_files)} artifact files")
        
        # Upload updated artifact back to WandB with proper naming
        if run and len(updated_files) > 0:
            # Use artifact_key for naming (e.g., "user_history_with_jsd" instead of "baseline_predictions_o3_history_logprobs_v4_with_jsd")
            updated_artifact_name = f"{artifact_key}_with_jsd"
            ...
            try:
                log_artifact(
                    ...
                    metadata={
                        "description": f"{artifact_key} with JSD metrics added to each review",
                        "original_artifact": artifact_name,
                        "artifact_key": artifact_key,
                        "num_files_updated": len(updated_files),
                        "num_reviews": len(results),
                        "mean_jsd": float(np.mean([r['jsd'] for r in results])) if results else 0.0
                    },
                    aliases=["latest"]
                )
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("JSD CALCULATION")
    logger.info("=" * 80)
    
    # Initialize W&B run
    run = init_wandb_run(
        run_name="calculate_jsd_pre_sgo",
        stage="Metrics and analysis",
        config={"description": "Calculate JSD for pre-SGO predictions"}
    )
    ...
    try:
        ...
        # Create output directory (JSD-only; uploaded as jsd_metrics_and_graphs in wandb)
        output_dir = BASE_DIR / output_config.get('jsd_directory', 'Metrics and analysis/artifacts/jsd')
        ...
        jsd_config_mode = jsd_config.get('mode', '1')
        artifacts_list = jsd_config.get('artifacts', [])
        
        if not artifacts_list:
            logger.error("No artifacts specified in config.jsd.artifacts. Please specify which artifacts to process.")
            return
        ...
        expected_count = {'1': 1, '2': 2, '3': 3, '4': 4}.get(jsd_config_mode, 1)
        if len(artifacts_list) != expected_count:
            logger.warning(f"Mode '{jsd_config_mode}' expects {expected_count} artifact(s), but {len(artifacts_list)} specified. Proceeding anyway.")
        
        logger.info(f"Mode: {jsd_config_mode}, Processing artifacts: {artifacts_list}")
        ...
        for artifact_key in artifacts_list:
            ...
            results = process_single_artifact(
                ...
                add_jsd_to_files=(jsd_config_mode == '1')  # Only add to files in mode 1
            )
        ...
        if jsd_config_mode == '2' and 'pre_sgo_context' in all_results and 'post_sgo_context' in all_results:
        ...
        if jsd_config_mode in ['2', '3', '4'] and len(all_results) >= 2:
            logger.info("\n" + "=" * 80)
            logger.info(f"GENERATING JSD COMPARISON GRAPHS: {len(all_results)}-way comparison")
            logger.info("=" * 80)
            
            comparison_dir = output_dir / f"jsd_comparison_{jsd_config_mode}way"
            ...
            for idx, (artifact_key, results) in enumerate(all_results.items()):
                ...
                artifact_means.append(mean_jsd)
                logger.info(f"{artifact_key}: Mean JSD = {mean_jsd:.6f} (n={len(jsd_values)})")
            
            # Calculate individual averages for each artifact
            avg_text_parts = []
            for artifact_key, results in all_results.items():
                jsd_values = [r.get('jsd') for r in results if r.get('jsd') is not None]
                artifact_avg = np.mean(jsd_values) if jsd_values else 0.0
                display_name = get_artifact_display_name(artifact_key)
                avg_text_parts.append(f"{display_name} Avg: {artifact_avg:.4f}")
            avg_text = "\n".join(avg_text_parts)
            ...
            ax.set_title(f'JSD Comparison: {len(artifact_labels)}-way (Overall)\n{avg_text}', 
                        fontsize=14, fontweight='bold', pad=20)
            ax.set_ylim(0, max(artifact_means) * 1.2 if artifact_means else 1.0)
            ax.grid(axis='y', alpha=0.3, linestyle='--')
            plt.tight_layout()
            plt.savefig(comparison_dir / "overall_jsd_comparison.png", dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # Upload all JSD graphs and results to WandB
        if run and output_dir.exists():
            logger.info("Uploading JSD metrics and graphs to WandB...")
            artifact_type = config.get("artifact_type", "result")
            try:
                log_artifact(
                    run=run,
                    artifact_name=config.get("output_artifacts", {}).get("jsd_metrics", "jsd_metrics_and_graphs"),
                    artifact_type=artifact_type,
                    artifact_path=str(output_dir),
                    metadata={"description": "JSD metrics, distribution graphs, per-cluster/tribe comparisons, and comparison results"},
                    aliases=["latest"],
                )
                logger.info("✓ Uploaded JSD metrics and graphs to WandB")
            except Exception as e:
                logger.warning(f"Failed to upload JSD artifact to WandB: {e}")
        
        # OLD CODE REMOVED - All processing now handled by the flexible artifact system above
        # All old hardcoded processing blocks have been removed
        # Processing is now controlled entirely by the artifacts list in config.yaml
        
        logger.info("\n" + "=" * 80)
        logger.info("JSD CALCULATION COMPLETE")
        logger.info("=" * 80)
        logger.info("JSD CALCULATION COMPLETE")
        logger.info("=" * 80)
